...
var PresentationModule = ( function ($, window, undefined) {
    ...
    function getMenuLink (slide, xIndex, yIndex, menuIndex) {
        // helper function that returns a complete menu link for a given slide
        return ('<a class="menu-link" href="#/' + xIndex + '/' + yIndex + '"><span>' + menuIndex + '</span><img src="img/menu-' + slide + '.png"></a>');
    };
        ...
        init: function () {
            ...
            topLevelSections.each(function (i) {
                ...
                if (subsections.length > 0) {
                    ...
                    subsections.each(function (subIndex) {
                        pipsHTML += '<li><a class="btn-goto" href="#/' + i + '/' + subIndex + '">&nbsp;</a></li>';
                    });
            ...
            if (offCanvasMenu.data('generate')) {
                ...
                topLevelSections.each(function (i) {
                    ...
                    if (subsections.length > 0) {
                        subsections.each(function (subIndex) {
                            appendMenuHtml += getMenuLink($(this).data('slide'), i, subIndex, pageIndex + '.' + subIndex);
                        });
                    } else {
                        // this slide is a single top level with no subsection... append it
                        var thisSingleLevel = $(this).find('> .swiper-slide');
                        appendMenuHtml += getMenuLink(thisSingleLevel.data('slide'), i, 0, pageIndex + '.0');
                    }
                });
            ...
            // event handler for off canvas menu links -- it was too sensitive to use 'tap' so reverting to native browser tap
            $('#menu-container .menu-link').on('click', function (e) {
                hideOffCanvas();
            });
